...
void createSortedCourseRegistrationOutput() {
    ...
    // Sort the vector of CourseRegistration objects by studentID
    std::sort(registrations.begin(), registrations.end(), [](const CourseRegistration& a, const CourseRegistration& b) {
        return std::strcmp(a.studentID, b.studentID) < 0;
    });

    // Open a file in write mode
    std::ofstream outFile("sorted_course_registrations.txt");
    if (!outFile) {
        std::cerr << "Failed to open file for writing." << std::endl;
        return;
    }

    // Write the sorted course registrations to the file
    for (const auto& reg : registrations) {
        outFile << reg << std::endl;
    }

    std::cout << "Sorted course registration records have been written to 'sorted_course_registrations.txt'." << std::endl;
